<?php

return [
   	'production' => false,
	'name' => 'JGB! Neat Design',
	'description' => 'Web & Mobile Development services',
	'url' => 'http://jgbnd.com.dev',
   	'blogUrl' => 'http://blog.jgbnd.com.dev',
   	'twitter' => 'https://twitter.com/jgbneatdesign',
   	'facebook' => 'https://www.facebook.com/jgbnddotcom',
   	'github' => 'https://github.com/jgbneatdesign',
   	'linkedin' => 'https://www.linkedin.com/in/jean-g%C3%A9rard-bousiquot-85a83689',
   	'owner' => 'Jean Gérard Bousiquot',
   	'email' => 'hello@jgbnd.com',
   	'tel' => '3647 8199',
   	'logo' => 'logo-jgbnd-small.png',
   	'logoSPNG' => 'logo-jgbnd-small-shadow.png',
   	'logoSJPG' => 'logo-jgbnd-small-shadow.jpg',
   	'clients' => [
      	[
         	'name' => 'Jerry Tardieu',
         	'image' => 'Jerry-Tardieu-500.jpg',
         	'website' => 'http://example.com'
      	],
     	[
         	'name' => 'Benjamin Boggess',
         	'image' => 'Benjamin-Boggess-500.jpg',
         	'website' => 'http://example.com'
      	],
      	[
         	'name' => 'Carel Pedre',
         	'image' => 'Carel-Pedre-500.jpg',
         	'website' => 'http://example.com'
      	],
     	[
         	'name' => 'Jean Marie Altema',
         	'image' => 'Jean-Marie-Altema-500.jpg',
         	'website' => 'http://example.com'
      	],
      	[
         	'name' => 'Boucicault',
         	'image' => 'boucicault.jpg',
         	'website' => 'http://example.com'
      	],

   	],
   	'testimonials' => [
      	[
         	'name' => 'Patric Attye',
         	'image' => 'Patrick-Attye-500.jpg',
   			'role' => 'CEO Facebook',
   			'text' => 'JGB! is the best in what they do. We can\'t believe how fast these guys are. They\'ve made a pretty good job pretty quickly',
      	],
      	[
         	'name' => 'Stephane Vincent',
         	'image' => 'vincent.jpg',
   			'role' => 'CEO Facebook',
   			'text' => 'JGB! is the best in what they do. We can\'t believe how fast these guys are. They\'ve made a pretty good job pretty quickly',
      	],

   	],

    'recentWorks' =>[
        [
            'name' => 'Hotel Royal Oasis',
            'image' => 'royal.jpg'
        ],
        [
            'name' => 'FD Strategy',
            'image' => 'fd.jpg'
        ],
        [
            'name' => 'Kingos',
            'image' => 'kingos.jpg'
        ],
        [
            'name' => 'Genesis Pro',
            'image' => 'genesis-pro.jpg'
        ],
        [
            'name' => '1Click',
            'image' => '1click.jpg'
        ],

                [
            'name' => 'Ti Kwen Pam',
            'image' => 'tikwenpam.jpg'
        ],
    ],

   	'img64' => 'data:image/svg+xml;base64,'
];